...
fun main() {
    val server = ServerBuilder
        .forPort(50051)
        .addService(FuncionarioEndpoitn())
        .build()

    server.start()
    server.awaitTermination()
}
//Instanciando classe para estender da interface criada no protobuf
class FuncionarioEndpoitn: FuncionarioServiceGrpc.FuncionarioServiceImplBase(){
    ...
    override fun cadastrar(request: FuncionarioRequest?, responseObserver: StreamObserver<FuncionarioResponse>?) {
        ...
        responseObserver?.onCompleted() //encerra a comunicação
    }
}